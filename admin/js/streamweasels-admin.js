(function($) {
    ...
    jQuery(document).ready(function(a) {
        ...
        if (jQuery('body').hasClass('toplevel_page_streamweasels-kick')) {
            ...
            function buildShortcode() {
                ...
                var shortcode = '[' + plugin + channels + limit + layout + embed + embedTitle + embedMuted + showOffline + showOfflineText + showOfflineImage + autoplay + featuredStream + title + subtitle + offlineImage + logoImage + profileImage + logoBgColour + logoBorderColour + maxWidth + tileLayout + tileSorting + liveInfo + tileBgColour + tileTitleColour + tileSubtitleColour + tileRoundedCorners + hoverColour + hoverEffect + ']';
		...
		if (jQuery('body').hasClass('kick-integration_page_streamweasels-kick-wall')) {
		...
		if (jQuery('body').hasClass('kick-integration_page_streamweasels-kick-status')) {
		...
		if (jQuery('body').hasClass('kick-integration_page_streamweasels-kick-feature')) {
		...
		if (jQuery('body').hasClass('kick-integration_page_streamweasels-kick-vods')) {
		...
		$(document).on('click', '.swki-notice button.notice-dismiss', function(event) {
			var data = {
				action: 'swki_admin_notice_dismiss',
			};
			$.post(ajaxurl, data, function(response) {
				console.log(response, 'SWKI - Notice Closed');
			});
		});
	
		$(document).on('click', '.swki-notice a.dismiss-for-good', function(event) {
			$('.swki-notice').hide()
			var data = {
				action: 'swki_admin_notice_dismiss_for_good',
			};
			$.post(ajaxurl, data, function(response) {
				console.log(response, 'SWKI - Notice Closed');
			});
